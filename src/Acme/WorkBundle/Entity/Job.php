...
class Job
{
    ...
    */
    ...
    public function estimator(\Acme\WorkBundle\Entity\Filter $filter) {
        if ( $this->neededNumber - $this->recruitedNumber > 0 ) {
            $eLanguage = $this->estimateLanguages($filter);
            $eCategory = $this->estimateCategories($filter);
            $eSalary = $this->estimateSalary($filter);
            $eJobPosition = $this->estimateJobPosition($filter);
            $jobid = $this->getID();
    ...
    public function estimateLanguages(\Acme\WorkBundle\Entity\Filter $filter) {
        $counter = 0;
        $alpha = 2;
        foreach ($filter->getLanguages() as $language) {
    ...
    public function estimateCategories(\Acme\WorkBundle\Entity\Filter $filter) {
        $counter = 0;
        $alpha = 2;
    ...
    public function estimateSalary(\Acme\WorkBundle\Entity\Filter $filter) {
        $alpha = 5;
        $beta = 4;
        if ($this->getSalary()->getId() == $filter->getSalary()->getId()) {
            return $alpha;
        } else if ($this->getSalary()->getId() - $filter->getSalary()->getId() == 1) {
            return $beta;
        }
        return 0;
    }

    public function estimateJobPosition(\Acme\WorkBundle\Entity\Filter $filter) {
        $alpha = 5;
        if ($this->getJobPosition()->getGroup() == $filter->getJobPosition()->getGroup()) {
            return $alpha;
        }
        return 0;
    }
